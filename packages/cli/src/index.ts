...
program
  ...
  .requiredOption("--server <url>")
  .requiredOption("--app-id <id>")
  .option("--dts-out <file>", "output declaration file", "socket-max.generated.d.ts")
...
function generateDTS(schema: any): string {
  ...
  for (const t of schema.topics || []) {
    ...
    for (const m of t.messages || []) {
      ...
      if (m.direction === 'publish' || m.direction === 'both') pub.push(`\"${m.name}\": ${tsType}`);
      if (m.direction === 'subscribe' || m.direction === 'both') sub.push(`\"${m.name}\": ${tsType}`);
    }
  ...
  lines.push("  type PublishType<T extends TopicName> = keyof Topics[T]['publish'];\n");
  lines.push("  type SubscribeType<T extends TopicName> = keyof Topics[T]['subscribe'];\n");
  lines.push("}\n");
  return lines.join("\n");
}
...
function generateTSHelpers(schema: any): string {
  ...
  lines.push("export type PublishType<T extends TopicName> = keyof Topics[T]['publish'];\n");
  lines.push("export type SubscribeType<T extends TopicName> = keyof Topics[T]['subscribe'];\n");
  lines.push(`export type PublishPayload<T extends TopicName, K extends PublishType<T>> = Topics[T]['publish'][K];\n`);
  lines.push(`export type SubscribePayload<T extends TopicName, K extends SubscribeType<T>> = Topics[T]['subscribe'][K];\n`);
  lines.push(`export type IncomingMessage<P> = { type: string; payload: P; ts?: number };\n`);
  lines.push(`export function typedPublish<T extends TopicName, K extends PublishType<T>>(client: { publish: any }, topic: T, type: K, payload: PublishPayload<T,K>, room?: string) {\n  return client.publish(topic as string, type as string, payload as any, room);\n}\n`);
  lines.push(`export function typedOnTopic<T extends TopicName, K extends SubscribeType<T>>(client: { onTopic: any }, topic: T, listener: (msg: IncomingMessage<SubscribePayload<T,K>>) => void) {\n  return client.onTopic(topic as string, listener as any);\n}\n`);
  return lines.join("\n");
}
...
function jsonSchemaToTs(s: any): string {
  ...
  if (Array.isArray(s.type)) {
    return s.type.map((t: string) => jsonSchemaToTs({ ...s, type: t } )).join(' | ');
  }
  switch (s.type) {
    ...
    case 'object': {
      ...
      const entries = Object.keys(props).map((k) => `${JSON.stringify(k)}${req.has(k) ? '' : '?'}: ${jsonSchemaToTs(props[k])}`);
      return `{ ${entries.join('; ')} }`;
    }
    default: return 'any';
  }
