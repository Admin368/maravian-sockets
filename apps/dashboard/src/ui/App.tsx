  ...
  return {
    ...
    topics: async (appId: string) => {
      const res = await fetch(
        base + `/api/topics?appId=${encodeURIComponent(appId)}`
      );
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
    rooms: async (appId: string) => {
      const res = await fetch(
        base + `/api/rooms?appId=${encodeURIComponent(appId)}`
      );
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
    schemaVersions: async (appId: string) => {
      const res = await fetch(
        base + `/api/schema/versions?appId=${encodeURIComponent(appId)}`
      );
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
    latestSchema: async (appId: string) => {
      const res = await fetch(
        base + `/api/schema/latest?appId=${encodeURIComponent(appId)}`
      );
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    },
            ...
            {lastAppKey && (
              <Panel title="New App Credentials">
                <div className="text-sm">
                  Save this App Key now. You won't be able to retrieve it later.
                </div>
                <pre className="text-xs mt-2">
                  {JSON.stringify(
                    { appId: appSel, appKey: lastAppKey },
                    null,
                    2
                  )}
                </pre>
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="text-2xl font-semibold tracking-wide">
        <span className="text-[var(--accent)]">●</span> Maravian Sockets
      </div>
      <div>
        {auth ? (
          <button
            className="px-3 py-1 bg-red-500/20 border border-red-500/40 rounded"
            onClick={onLogout}
          >
            Logout
          </button>
  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex gap-2">
        <input
          className="bg-transparent border border-white/10 rounded px-2 py-1 flex-1"
          placeholder="email"
          value={email}
        ...
        />
        <button
          className="px-3 py-1 bg-[var(--accent)]/20 border border-[var(--accent)]/40 rounded"
          onClick={async () => {
            try {
              setErr(null);
...
function AppsBar({
  ...
  return (
    <div className="card p-4 flex items-center gap-3">
      ...
      />
      <button
        className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/40 rounded"
        onClick={() => newId && onCreate(newId)}
      >
        Create app
      </button>
    </div>
